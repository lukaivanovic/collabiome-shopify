    ...
    if (this.tagName === "CART-DRAWER-ITEMS") {
      return fetch(`${routes.cart_url}?section_id=cart-drawer`)
        ...
        .then((responseText) => {
          ...
          const sourceInner = html.querySelector("#CartDrawer .drawer__inner");
          const targetInner = document.querySelector(
          ...
          refreshRebuyWidgets(targetDrawer || document);
        })
        .catch((e) => {
          console.error(e);
        });
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        CartPerformance.measure(`${eventTarget}:paint-updated-sections`, () => {
          ...
          if (cartDrawerWrapper) {
            refreshRebuyWidgets(cartDrawerWrapper);
          }
...
function refreshRebuyWidgets(context = document) {
  if (!context) return;
  if (!context.querySelector("[data-rebuy-id]")) return;
  if (typeof window.Rebuy === "undefined") return;
